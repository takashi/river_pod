...
このガイドを読む前に [read about Providers](/docs/concepts/providers) を確認してください。

このガイドでは、どのようにプロバイダが消費されるかを見ていきます。

プロバイダの読み方には複数の方法があり、それぞれの基準によって少しずつ変わっていきます。

プロバイダの読み方は複数あり、さまざまな基準によって少しずつ変化しています。

長くなりましたが、ここでは、プロバイダを読むために何を使うかを決めるための決定グラフをご紹介します:

![Decision graph for reading providers](/img/intro/reading.svg)

次に、個々のケースを見て、その仕組みを紹介します。
このガイドでは、次のようなプロバイダがあると考えます:
...
待ち受けたいプロバイダによって、どのような値を待ち受けるかについて、複数の選択肢がある場合があります。

例として、下のような [StreamProvider] について考えてみましょう:

```dart
final userProvider = StreamProvider<User>(...);
```

この `userProvider` を読み取るとき、以下の値を読み取ることができます:

- `userProvider` 自体を待ち受けることで、現在の状態を同期的に読み取る:
...
- `userProvider.stream` を待ち受けることで紐付いた [Stream] を取得する:
...
- `userProvider.last` を待ち受けることで放出された最新の値を解決する [Future] を取得する:
...
詳細は、[API reference](https://pub.dev/documentation/riverpod/latest/riverpod/riverpod-library.html) から各プロバイダのドキュメントを参照してください。
